        ...
        var procedure_view = ""
            if (data.length >= 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    procedure_view += '<td style="display:none"><span style="color:#017b8b";>Status:</span><span>'+ alr.status + '</span></td>';
...
//hare lab details or send  share lab details */
    ...
    load_labtests = function(){
        ...
        if (data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                var alr = data[i]
                diagnostic_view += '<tr>';
                diagnostic_view += '<td class="testcode"><span style="color:#017b8b";>Test Name:</span><span>'+ alr.test_id.codes + ' '+alr.test_id.description+ '</span></td>' ;
                ...
                diagnostic_view += '<td class="testcode"><span style="color:#017b8b";>Reference Range:</span><span>'+ alr.test_id.ref_range+ '</span></td>' ;
                ...
                if(alr.result == null){
                diagnostic_view += '<td style="display:none" class="testcode"><span style="color:#017b8b";>Units:</span><span>'+ alr.test_id.units +'</span></td>' ;
                }
                else{
                diagnostic_view += '<td style="display:block" class="testcode"><span style="color:#017b8b";>Units:</span><span>'+ alr.test_id.units +'</span></td>' ;
                  }
                if(alr.result == null){
                diagnostic_view += '<td style="display:none" class="testcode"><span style="color:#017b8b";>Result:</span><span>'+ alr.result + '</span></td>' ;
                }
                else{
                   diagnostic_view += '<td style="display:block" class="testcode"><span style="color:#017b8b";>Result:</span><span>'+ alr.result + '</span></td>' ;
                }
...
//                    '<a href="javascript:void(0)" data-id="' + alr.id + '" class="diagnosticdelete">Delete</a></td>';
            ...
            printdiagnostic += '<button style="color:#ffff;margin-left:10px;"><a style="color:#ffff" href="/testdetails/'+String(pat_id)+'">Generate Test Report</a></button>'


            $("#diagnosticlab").html(diagnostic_view);
            $("#printtestdetails").html(printdiagnostic);
        }
    });
    }
...
//                jes=jes[jes.length - 1]
                ...
                medications_view += '<td><span style="color:#017b8b;">Composition name:</span><span>'+ alr.compositionName+ '</span></td>';
              ...
              //  medications_view += '<td><span style="color:#017b8b;">End date:</span><span>'+  alr.enddate  + '</span></td>';
                medications_view += '<td><span style="color:#017b8b;">Frequency:</span><span>'+  alr.frequency  + ' (MN-AF-EN-NT) / '+alr.withfood+'</span></td>';
                medications_view += '<td><span style="color:#017b8b;">Duration:</span><span>'+ alr.duration + '</span></td>';

                medications_view += '<td><span style="color:#017b8b;">Usage Directions:</span><span>'+ alr.usageDirection + '</span></td>';
...
//            medicationbtn +='<button style="color:#ffff;margin-left:10px;"><a style="color:#ffff" href="/medicationdetails/'+String(pat_id)+'">Generate Medicine Prescription</a></button>'
//            $("#printmedicationbtn").html(medicationbtn);
     ...
     var new_dialog = function (type, row,medications_id) {
      ...
      function get_medications_data_for_edit(row) {
            ...
            $("#quantity").val($(row.children().find('span').get(5)).text());//frequency
            $("#frequency").val($(row.children().find('span').get(7)).text());
            // $("#medstartdate").val($(row.children().find('span').get(7)).text());
            $("#duration").val($(row.children().find('span').get(7)).text());
        ...
        function edit_medications_data(id) {
            ...
            var medications_data = {
                ...
                "enddate": medenddate,
                "usageDirection": directionstoussage,
                "refills": refills,
        ...
        function save_medications_data() {
            var medicationname = $("#medicationname").val();
            medicationname = medicationname.split(":")
            alert(medicationname[0])
            var compositionName= $("#compositionName").val();
            var dossage = $("#dossage").val();
            var quantity = $("#quantity").val();
            // var medstartdate = $("#medstartdate").val();
            // var medenddate = $("#medenddate").val();
             ...
             if (data.length > 0) {
                ...
                vaccine_view += '<tbody style="border:5px">';
                    for (var i = 0; i < data.length; i++) {
                        ...
                        vaccine_view += '<td style="display:none"><span style="color:#017b8b";>Status:</span><span>'+ alr.v_status + '</span></td>';
...
//                            '<a href="javascript:void(0)" data-id="' + alr.id + '" class="vaccinedelete">Delete</a></td>';
                ...
                vaccine_view += '</tbody>';
//                        printvaccinebtn+='<button style="color:#ffff;margin-left:10px;"><a style="color:#ffff" href="/vaccinedetails/'+String(pat_id)+'">Generate Vaccine Presription</a></button>'


                $("#vaccine").html(vaccine_view)
//                $("#printvaccine").html(printvaccinebtn);
             }
         });
     }
...
//                    attachfile_view += '<td><span style="color:#017b8b"; >Description:</span><span>'+ alr.description + '</span></td>';
                    ...
                    attachfile_view += '<td><span style="color:#017b8b"; >Description:</span><span><a style="color:green !important; font-weight:bold;"  target="_blank" href=" '+ alr.source +' ">'+ alr.description + '</a></span></td>';
...
//            }
            ...
            var data = new FormData(form);
            data.append("patient",patient_id);
            data.append("appointment",appointmentId)
