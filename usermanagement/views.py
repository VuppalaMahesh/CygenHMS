...
def redirectLoginview(request,name):
    ...
    if userType == 1:
        ...
        return render(request, "ehr_patient_list_in_physician.html",
                      {'appoint': a, 'Lab_Technician': Lab_Technician, 'receptionist': receptionist, 'admin': admin,
                       'Pharmacist': Pharmacist, 'Nurses': Nurses, 'physician': physician, 'Patient': Patient})
    elif userType == 7:
        lab = Profile.objects.all().filter(user_type_id=1)
        Lab_Technician = len(lab)
        recep = Profile.objects.all().filter(user_type_id=2)
...
def patient_listehr_physian_view(request):
    ...
    if not request.user.is_authenticated:
        return HttpResponse('<h3>your session has expired please relogin</h3>')
    return render(request, "ehr_patient_list_in_physician.html",context)
...
class get_latestPatient_listview(ListAPIView):
    ...
    # def get_queryset(self):
    #     latest = date.today()
    #    queryset = self.queryset.filter(UHID__in=Subquery(
    #    Appointmentrequest.objects.filter(bookngdate=latest).values("patient__UHID")))
...
def healthrecordphysicanview(request,pat_id):
    ...
    if obj == True:
        ...
        d = request.GET["bookingdate"]
        print(str(d),'jhgvjhgjh')
        try:
            app_id = Appointment.objects.get(bookingdate=str(d), patient=patient)
        except Appointment.DoesNotExist:
            app_id=None
        a = (profile.first_name + ' ' + profile.middle_name + ' ' + profile.last_name)
        print(app_id.id)
        return render(request, 'ehr_physican.html', {"app_id":app_id.id,"UHID": patient.UHID, "patient": patient.UHID,"patient_name":a,"pat_id":pat_id})
    else:
        return HttpResponse("not have authority")
...
def healthrecordnurseview(request,pat_id):
    ...
    if obj == True:
        ...
        return render(request, 'ehr_nurse.html', {"app_id":app_id,"UHID": patient.UHID, "patient": pat_id,"patient_name":a})
    else:
        return HttpResponse("not have authority")
...
def OPdrecordview(request,pat_id):
    ...
    # doc = request.user.id
    patient = Patient.objects.get(pat_id=pat_id)
    pat_profile = Profile.objects.get(id=pat_id)
    d = date.today()
    app_id = Appointment.objects.get(bookingdate=d, patient=patient)
    doc = app_id.doctor
    doctorname = app_id.doctor.pro.first_name + ' ' + app_id.doctor.pro.middle_name + ' ' + app_id.doctor.pro.last_name
    app_id = Appointment.objects.get(doctor=doc,bookingdate=d, patient=patient)
    # UHID=patient.UHID
    # patient_id
    pat_name= pat_profile.first_name + pat_profile.middle_name + pat_profile.last_name
    return render(request, 'opdrecordview.html', {"UHID": patient.UHID, "patient": pat_id,'pat_name':pat_profile,"app_id":app_id})
    # else:
    #     return HttpResponse("not have authority")
...
class appointedpatients(ListAPIView):
    lookup_field = 'pat_id'
    queryset = Patient.objects.all()
    serializer_class = PatientSerializer
    def get(self, request, *args, **kwargs):
        ...
        if user is None:
            ...
            latest = date.today()
            appointemnts = Appointment.objects.filter(bookingdate=latest)
            doctorids = [(i.patient.UHID, i.id) for i in appointemnts]
            print(doctorids)
            lis = []
            for i in doctorids:
                print("Dfdffdd")
                queryset = self.queryset.get(UHID=i[0])
                dic = model_to_dict(queryset)
                for key, val in dic.items():
                    if key == "pat":
                        pro = Profile.objects.get(id=val)
                        dic_pro = model_to_dict(pro)
                        dic["pat"] = dic_pro
                    elif key == "doc_link":
                        lisd = []
                        for g in dic["doc_link"]:
                            lisd.append(model_to_dict(g))
                        dic["doc_link"] = lisd
                dic["appointment"] = i[1]
                dic["UHID"] = i[0]
                lis.append(dic)
        ...
        us = Profile.objects.get(user_id=user)
        usertype = us.user_type_id
        if usertype == 6:
            doc = Doctors.objects.get(doc_id=user)
            queryset = doc.patient_set.all()
            print(queryset,'jjjjjjjjjjjjj')
            latest = date.today()
            queryset = queryset.filter(UHID__in=Subquery(
                Appointment.objects.filter(bookingdate=latest).values("patient__UHID")))
            page = self.paginate_queryset(queryset)
            if page is not None:
                serializer = self.get_serializer(page, many=True)
                return self.get_paginated_response(serializer.data)
...
def medicationdetails(request,pat_id):
    # obj = permission_req(Isreceptionist(),Isdoctor(),Isnurse(), request, view=None)
    # if obj == True:
    patient = Patient.objects.get(pat_id=pat_id)
    d = date.today()
    app_id = Appointment.objects.get(bookingdate=d, patient=patient)
    doc = app_id.doctor
    doctorname = app_id.doctor.pro.first_name+' '+app_id.doctor.pro.middle_name+' '+app_id.doctor.pro.last_name
    app_id_doc = Appointment.objects.get(doctor=doc,bookingdate=d,patient=patient)
    print(app_id_doc,'ksjdhfkj')
    appointmentDate = app_id.bookingdate.strftime("%Y-%m-%d")
    return render(request,"printmedication.html",{"pat_id":pat_id,"patient":patient,"appointmentDate":appointmentDate,"app_id":app_id_doc,"doctor":doctorname})

def vaccinedetails(request,pat_id):
    # obj = permission_req(Isreceptionist(),Isdoctor(),Isnurse(), request, view=None)
    # if obj == True:
    patient = Patient.objects.get(pat_id=pat_id)
    d = date.today()
    ...
    app_id_doc = Appointment.objects.get(doctor=doc,bookingdate=d,patient=patient)
    appointmentDate = app_id.bookingdate.strftime("%Y-%m-%d")
    return render(request,"printvaccinedetails.html",{"pat_id":pat_id,"app_id":app_id_doc,"patient":patient,"appointmentDate":appointmentDate,'doctor':doctorname})

def testdetails(request,pat_id):
    # obj = permission_req(Isreceptionist(),Isdoctor(),Isnurse(), request, view=None)
    # if obj == True:
    ...
    app_id_doc = Appointment.objects.get(doctor=doc,bookingdate=d,patient=patient)
    appointmentDate = app_id.bookingdate.strftime("%Y-%m-%d")
    return render(request,"printTestdetails.html",{"pat_id":pat_id,"app_id":app_id_doc,"patient":patient,"appointmentDate":appointmentDate,'doctor':doctorname})



class searchpatdata(ListAPIView):
    lookup_field = 'id'
    queryset = Patient.objects.all()
    serializer_class = PatientSerializer
    def get(self, request, *args, **kwargs):
        ...
        print(key)
        queryset = self.queryset.filter(pat__first_name__icontains=key) | self.queryset.filter(UHID__icontains=key)
        print(queryset)
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = self.get_serializer(page, many=True)
...
def Doctoravailability(request,id):
    data =DoctorSlots.objects.filter(doctor_id=id)
    if data:
        available=1
    else:
        available=0
    return render(request,'doctoravailability.html',{"doc_id":id,"available":available})
...
def billsview(request,appointment_id):
    ...
    uhid=p.UHID
    return render(request, 'bills.html',{"uhid":uhid,"app_id":app_id,"name":name})

def totalhealthrecordphysicanview(request,pat_id):
    pat_id=pat_id
    patient = Patient.objects.get(pat_id=pat_id)
    print(patient.UHID)
    context = {"UHID":patient.UHID,"pat_id": pat_id}
    return render(request,"ehr_total.html",context)

class appointedInpatients(ListAPIView):
    lookup_field = 'pat_id'
    queryset = Patient.objects.all()
    serializer_class = PatientSerializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        if usertype == 6:
            ...
            self.queryset = self.queryset
            appointemnts = Appointment.objects.filter(Q(flag=1)&Q(appointmentType=1))
            doctorids = [(i.patient.UHID, i.id) for i in appointemnts]
            print(doctorids,'kkkkkkkkkkkk')
            lis = []
            for i in doctorids:
                queryset = self.queryset.get(UHID=i[0])
                dic = model_to_dict(queryset)
