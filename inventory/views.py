...
def product_list(request):
    ...
    """
    if request.method == 'GET':
        products = Product.objects.all().filter(productStatus='ACTIVE')
...
def product_detail(request, pk):
    ...
    if request.method == 'GET':
        ...
        Product.objects.all().filter(pk=pk).update(productStatus='INACTIVE')
        return Response(status=status.HTTP_204_NO_CONTENT)
...
def purchase_order_date_range(request):
    ...
    if request.method == 'GET':
        start_dt = request.data['startDate']
        end_dt = request.data['endDate']
        orders = PurchaseOrder.objects.all().filter(orderDate__range=(start_dt, end_dt))
...
def vendor_list(request):
    ...
    if request.method == 'GET':
        vendors = Vendor.objects.all().filter(vendorStatus='ACTIVE')
...
def vendor_detail(request, pk):
    ...
    if request.method == 'GET':
        ...
        Vendor.objects.all().filter(pk=pk).update(vendorStatus='INACTIVE')
        return Response(status=status.HTTP_204_NO_CONTENT)
